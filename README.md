...
- **환승 경로 탐색** — 두 역을 이름으로 찾아 지정하면 **최소 시간 / 최소 환승** 후보를 찾아 보여 줍니다. 소요시간·환승 횟수·정거장 수와 노선 배지 사슬을 함께 싣습니다. 출발역만 골라 실시간 도착만 볼 수도 있습니다.
- **실시간 도착 안내** — 서울 열린데이터광장의 실시간 도착정보를 방향별로 보여 줍니다. 폴링 사이에도 카운트다운이 초 단위로 흐릅니다.
- **구간별 승하차·환승 알림** — 경로를 고르면 구간마다 "N정거장 전"과 "곧 하차/환승" 로컬 알림을 예약합니다. 환승역에 도착하면 다음 열차 대기 구간으로 넘어가 실시간 도착정보를 다시 씁니다.
...
- **노드** = (운행 계통, 역 인덱스) 794개. **승차 간선** = 인접 쌍, 가중치는 노선 평균 소요시간. 순환선은 끝과 처음도 잇습니다.
...
- 경로 탐색의 비용 모델은 추정입니다. 데이터셋에 구간별 소요시간도, 환승 통로 도보 시간도, 급행 운행 계통도 없습니다. 노선당 평균 소요시간과 `src/services/routing/cost.ts` 의 상수로 계산하므로, 예를 들어 김포공항 → 강남은 9호선 급행을 고려하지 못합니다. 실측값이 생기면 그 파일만 고치면 됩니다.
