...
매일 아침저녁 출퇴근에서 **잠들어도 되는** 지하철 앱입니다. 수도권 전철 실시간 도착·열차 위치와
승차·하차·환승 **알람**, 빠른 환승 칸 안내, 늘 타는 **내 경로**와 **출퇴근 루틴**을 제공하는 React Native(Expo) 앱과
그 데이터를 공급하는 C# 백엔드([`backend/`](backend/README.md))입니다. 노선도 앱이 아닙니다.

## 기능

- **홈** — 진행 중인 여정 배너, 출발·도착 검색과 최근 검색, 집·회사·즐겨찾기 칩(가장 빨리 오는 열차까지의 시간이 얹힘), 내 주변 역, 운행 공지, 노선별 열차 위치 진입점.
- **환승 경로 탐색** — 두 역을 지정하면 **추천 / 최소 시간 / 최소 환승 / 최소 정거장** 후보와, 추천 경로의 노선을 하나씩 피한 **대안**("2호선 제외")을 보여 줍니다. 경유역과 노선 제외를 지정할 수 있습니다. 서울교통공사 **역간 실측 운행시간**과 **환승 도보 시간**으로 계산하고, 각 환승마다 **"3-2 칸 하차 → 5-1 칸 승차"** 같은 빠른 환승 칸을 함께 싣습니다.
- **내 경로** — 늘 타는 길이 어느 기준에도 안 잡히면 그 경로를 **저장**합니다. 같은 출발·도착을 검색하면 저장 경로가 추천보다 앞에 오고, 홈에서 두 번 눌러 바로 시작합니다. 노선 데이터가 바뀌어도 같은 모양의 경로를 다시 찾습니다.
- **출퇴근 루틴** — "평일 07:30 출근" 처럼 요일·시각을 정해 두면 그 시각에 **여정 시작 알림**이 오고, 알림의 [여정 시작] 한 번으로 저장 경로의 하차 알람이 시작됩니다. 그 시간대에 앱을 열어도 시작할지 묻습니다(자동 시작 옵션).
- **실시간 도착·열차 위치** — 역별 도착 카운트다운과, 노선을 세로 트랙으로 그린 **열차 위치 화면**(방향 토글, 급행·막차 표시).
- **승차·하차·환승 알람** — 탈 열차가 1분 안에 오면 **승차 알림**, 하차·환승역은 "N정거장 전" 예비 알람과 도착 1분 전 알람을 **전용 알람음**으로 보냅니다(Android 는 방해 금지 우회, iOS 는 time-sensitive). 알림의 [승차했어요]·[갈아탔어요]·[확인] 버튼으로 앱을 열지 않고 진행하고, 승강장에 있던 열차가 떠나면 **자동으로 승차를 감지**해 묻거나 넘깁니다. 승차 후에는 **승차한 열차의 실시간 위치**로 진행 상황을 계산하고, 반대 방향 열차면 경고합니다.
...
src/                 Expo 앱 (expo-router)
  app/(tabs)/        홈 · 여정 · 설정
  app/routines/      출퇴근 루틴 목록·편집
  app/line/[lineId]  노선 열차 위치
  services/alerts    여정 진행·알림 예약 (순수 모듈은 pnpm test 로 검증)
  services/routing   경로 탐색 (프로파일·대안·경유)
  services/routes    저장 경로 (내 경로)
  services/routines  출퇴근 루틴 (WEEKLY 리마인더)
...
- 승차한 뒤에는 도착정보를 쓰지 않고, 열차 위치만 30초마다 받습니다 (아래 참고).
- 설정의 데이터 소스에서 "모의"를 고르면 호출을 완전히 멈출 수 있습니다.

## 알림이 동작하는 방식

### 알람으로 울리기

하차·환승 알림은 잠든 사람을 깨워야 하므로 보통 알림과 다르게 보냅니다.

- **알람음** `assets/sounds/alarm.wav` (`pnpm generate-alarm-sound` 산출물)을 번들합니다. iOS 는 알림 콘텐츠의 `sound`, Android 는 채널 `trip-alarm-v2` 의 사운드로 지정됩니다.
- **Android** 채널은 `importance MAX` · `usage ALARM`(알람 볼륨) · `bypassDnd`(방해 금지 우회)입니다. 정확한 시각에 울리도록 `SCHEDULE_EXACT_ALARM`/`USE_EXACT_ALARM` 권한을 선언합니다 — Android 12 는 설정에서 "알람 및 리마인더"를 켜야 하며 설정 화면에 진입점이 있습니다.
- **iOS** 는 `interruptionLevel: 'timeSensitive'` 를 보냅니다. 집중 모드를 뚫으려면 엔타이틀먼트가 필요한데 무료 Personal Team 서명에서 거부될 수 있어 옵트인입니다: `IOS_TIME_SENSITIVE=1 pnpm prebuild`. 없으면 OS 가 조용히 일반 알림으로 낮춥니다. 무음 스위치는 따르므로 출퇴근 중에는 무음을 풀어 두세요. 긴급 알림(critical)은 Apple 승인이 필요해 쓰지 않습니다.
- 알림에 **버튼**이 있습니다: 승차 알림 [승차했어요]·[이 열차 아님], 예비 [확인], 환승 [갈아탔어요]·[확인], 도착 [확인 (여정 종료)]. 앱이 죽어 있다가 버튼으로 켜져도 `getLastNotificationResponseAsync` 로 이어받습니다.
- 도착·환승 알림은 확인할 때까지 트레이에 남습니다(sticky). 반복 사다리·앱 내 알람 모드는 넣지 않았습니다 (후속 옵션).

### 예약 시각의 규칙

- **정적 추정으로는 이미 잡힌 알림을 옮기지 않습니다.** 도착정보가 없을 때의 추정("지금 + 구간 소요")은 매 폴링마다 뒤로 밀리므로, 그 값으로 재예약하면 승강장에서 기다리는 동안 알람이 무한히 미뤄집니다. 정적 추정은 아직 아무것도 잡히지 않았을 때(여정 시작 직후) 안전망으로만 씁니다.
- 승차 전 백그라운드에서는 아무것도 계산하지 않습니다(Android 는 JS 가 계속 돕니다).
- 예약 시각이 지난 알림은 OS 가 발화한 것으로 소비합니다. 죽어 있는 동안 도착 알림이 지났으면 다시 켤 때 여정을 끝내고, 늦은 "도착" 알림을 한 번 더 띄우지 않습니다.
- 승차 취소는 그 구간의 발화 기록까지 지웁니다 — 그래야 제대로 탄 열차에서 알림이 다시 잡힙니다.
- 승강장에 있던 열차가 도착 목록에서 사라지면 "타셨나요?" 하고 묻고, 열차 위치 API 가 그 열차를 다음 역에서 보면 자동으로 승차 처리합니다. 승차 후 열차 위치가 두 번 연속 반대 방향이면 경고합니다.
...
목표역(하차역 또는 환승역) 좌표를 아는 경우 지오펜스(예비 500m, 도착 300m)를 걸어 진입 시 즉시 알립니다.
지하 구간에서는 GPS 가 잡히지 않으므로 어디까지나 보조 수단입니다.
두 경로 중 먼저 도달한 쪽이 알림을 소비하고 나머지 예약은 취소합니다 — 앱이 죽어 있어도 지오펜스 태스크가 저장소의 여정에 기록하므로 같은 사건이 두 번 울리지 않습니다. 재부팅·복구 뒤에는 지오펜스를 다시 겁니다.
...
- 후보는 가중치를 바꿔 Dijkstra 를 네 번 돌려 만듭니다 — **추천**(환승 1회 = 3분, 실시간 추적 불가 노선 = 2분 불이익), **최소 시간**, **최소 환승**(환승 1회 = 7분), **최소 정거장**(정거장 1개 = 120초). 같은 경로가 여러 기준에서 나오면 하나로 합쳐 "추천 · 최소 시간" 처럼 표시합니다. 그 뒤 추천 경로가 탄 계통을 하나씩 피해 **대안**을 찾습니다(최선의 1.5배 이내, 전체 6개까지). 797 노드에서 1회가 1ms 남짓이라 k-shortest 알고리즘이 필요 없습니다.
- **경유역**은 두 번 탐색해 잇고, **노선 제외**는 그 계통의 간선을 건너뜁니다.
- **저장 경로**(`src/services/routes/saved.ts`)는 계통·방향·승하차역을 고정해 저장합니다. 같은 출발·도착을 검색하면 맨 앞에 "내 경로"로 옵니다. 데이터셋이 바뀌어 인덱스가 어긋나면 같은 모양의 후보를 다시 찾아 갱신하고(refreshed), 없으면 다시 저장하라고 안내합니다(unavailable).

`graph.ts` 에는 **런타임 import 가 하나도 없습니다** (`eta.ts` 와 같은 규율). 그래서 `scripts/verify-routes.mjs` 와 `pnpm test` 가 이 파일을 그대로 읽어 실제 데이터셋으로 경로를 검증합니다 — `stations.ts` 는 `lines.json` 을 import 하기 때문에 Node 에서 읽히지 않습니다. 역명 정규화·방향·정거장 수 계산의 원본도 같은 이유로 `graph.ts` 에 있고 `stations.ts` 가 감싸 씁니다.
...
pnpm test                               # node:test — 알림 시각·진행 계산·상태 전이·경로 탐색·루틴 시각의 순수 모듈
pnpm verify-data                        # 데이터셋 불변식 검사
...
`pnpm test` 는 의존성 없이 Node 의 타입 스트리핑으로 `src/**/*.test.ts` 를 돌립니다. `scripts/test/hooks.mjs` 가 `@/` 별칭과
확장자 없는 상대 경로를 풀어 주므로, 테스트할 수 있는 모듈은 **런타임 import 가 순수 TS 로만 이어지는** 모듈입니다
(`eta.ts`, `progress.ts`, `trip-state.ts`, `auto-board.ts`, `graph.ts`, `saved.ts`, `routines/schedule.ts`, `notifications/response.ts`).
expo-*·react-native·JSON 을 import 하는 모듈은 그 경계 바깥에 둡니다. Node 22.18+ 가 필요합니다(`.nvmrc`).
...
- 구간 실측 운행시간은 서울교통공사 운영 구간(1~8호선 일부)에만 있고, 나머지는 노선 평균입니다. 급행 운행 계통과 시각표 기반 경로 탐색은 아직 없습니다 — 시각표는 백엔드가 있을 때 경로 확인 화면의 다음 열차·막차 안내에만 씁니다(1~9호선).
- 알림은 전부 기기 로컬입니다. 서버 푸시가 없으므로 OS 가 앱의 예약 알림을 지우면(저장 공간 정리, 강제 종료 후 일부 제조사) 알람도 사라집니다. 반복 알림 사다리·앱 내 알람 모드는 넣지 않았습니다.
- 출퇴근 루틴의 "자동 시작"은 앱을 열었을 때만 동작합니다. 모바일 OS 는 앱을 예약 실행해 주지 않으므로, 알림의 [여정 시작] 버튼이 실질적인 자동 시작입니다.
...
앱: Expo SDK 57 · React Native 0.86 · React 19.2 · expo-router · TypeScript(strict) · react-native-svg · expo-notifications · expo-location + expo-task-manager · expo-intent-launcher · @react-native-community/datetimepicker · expo-dev-client · EAS Build · node:test
